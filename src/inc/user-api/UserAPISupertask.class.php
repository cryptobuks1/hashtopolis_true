...
class UserAPISupertask extends UserAPIBasic {
  ...
  private function createSupertask($QUERY) {
    ...
    SupertaskUtils::createSupertask($QUERY[UQueryTask::SUPERTASK_NAME], $QUERY[UQueryTask::PRETASKS], $QUERY[UQueryTask::TASK_PRIORITY]);
    $this->sendSuccessResponse($QUERY);
  }
  ...
  private function getSupertask($QUERY) {
    ...
    $response = [
      ...
      UResponseTask::TASK_PRIORITY => $supertask->getPriority(),
      UResponseTask::PERCENT => $supertask->getPercent(),
      UResponseTask::CREATE_TIME => $supertask->getCreateTime(),
      UResponseTask::CHUNK_STATE => $supertask->getStatus()
    ];
    foreach ($pretasks as $pretask) {
      ...
      $sbis_count += $pretask->getCurSbisCount();
    }
    $response[UResponseTask::PRETASKS] = $taskList;
    $response[UResponseTask::SBIS_COUNT] = $sbis_count;
